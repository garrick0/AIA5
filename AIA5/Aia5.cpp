...
Mat Aia5::calcCompLogL(vector<struct comp*>& model, Mat& features){
    ...
    //LikelyMat holds the log likelihoods of the model for each feature
    Mat likelyMat = Mat(features.cols, model.size(),CV_32FC1);
    
    //Loop through each feature
    
    for (int ft = 0; ft < features.cols; ft++) {
        //Loop through each model attribute
        for (int i = 0; i < model.size(); i++) {
            //Get variables for mathematics (use arrow because pointer)
            Mat meanMat = model.at(i)->mean.clone();
            Mat varMat = model.at(i)->covar.clone();
            
            //For storage
            Mat logVariance = Mat(varMat.size(),CV_32FC1);
            
            //Perform math from slides
            
            //Take determinant of covariance matrix
            double deter = determinant(varMat);
            //Take log of covariance and divide by -.5 to get first term
            
            double termOne = -.5* log(deter);
           
            
            
            //Get values for second term
            //Remove mean
            Mat xRem = features.col(ft) - meanMat;
            
           // cout << xRem << endl;
            
            //Transpose xRem
            Mat xRemTransp = xRem.t();
            //Invert covariance
            Mat varInvert = varMat.inv();
            
            
            //Multiply to get second term in equation
            Mat termTwo = -.5 * ((xRemTransp * varInvert) * xRem);
            
            //cout << "Term two is " << ft * 5 + i << endl;
            //cout << termTwo << endl;
            
            //Get z value
            double alpha = model.at(i)->weight;
            //Take log of alpha for term 3
            
            // DO NOT THINK THIS IS RIGHT
            double termThree = log(alpha);
            
            
            //NEED TO DOUBLE CHECK TERM THREE
            termThree = - (features.rows / 2.0) * log(2 * M_PI);
            //cout << termThree << endl;
            
            double result = termOne + termTwo.at<float>(0,0) + termThree;
            
            
            likelyMat.at<float>(ft,i) = result;
            
            //cout << "Components are " << termOne << ", " << termTwo.at<float>(0,0) << ", " << termThree << endl;
            cout << result << endl;
...
void Aia5::run(void){
    ...
    readImageDatabase("/Users/Sam/Documents/Developer/cpp/Aia5/img/train/in", trainImgDB);
...
void Aia5::genFeatureProjection(vector<Mat>& imgDB, vector<PCA>& featSpace, int vectLen){
    ...
    for(vector<Mat>::iterator cat = imgDB.begin(); cat != imgDB.end(); cat++, c++){
        //Move through each image and convert to PCA
